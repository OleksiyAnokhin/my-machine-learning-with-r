...
survive_data <- read_csv("gender_submission.csv")
head(survive_data)
```

**Join data**

Why do we need to join train data with survive data? Because we do not have a boolean column in our test dataset. Hence, we will not be able to analyze properly.

I will use the `inner_join()` here. 

[Source:](https://dplyr.tidyverse.org/reference/join.html)

```{r, message = FALSE, warning = FALSE}
test_data_joined <- inner_join(test_data, survive_data)
test_data_joined
```

Now we added one column with a survival value 1 or 0.
...
```{r, message = FALSE, warning = FALSE}
predicted <- predict(logit_model, test_data_joined, type = "response") 
summary(predicted)
```

Now let's try to predict, using some specific values - sex and Pclass.
...
```{r, message = FALSE, warning = FALSE}
optCutOff <- optimalCutoff(test_data_joined$Survived, predicted)[1]
optCutOff
```
...
In our case it will not work, we do not have 1 or 0 in train data. 

```{r, message = FALSE, warning = FALSE}
misClassError(test_data_joined$Survived, predicted, threshold = optCutOff)
```

We have 0, so it is a great result, nothing was misclassified. 
...
```{r, message = FALSE, warning = FALSE}
plotROC(test_data_joined$Survived, predicted)
```

Looks like our model predict perfectly. 
...
```{r, message = FALSE, warning = FALSE}
Concordance(test_data_joined$Survived, predicted)
```
...
```{r, message = FALSE, warning = FALSE}
sensitivity(test_data_joined$Survived, predicted, threshold = optCutOff)
specificity(test_data_joined$Survived, predicted, threshold = optCutOff)
```

**Confusion Matrix**

```{r, message = FALSE, warning = FALSE}
confusionMatrix(test_data_joined$Survived, predicted, threshold = optCutOff)
```
